-- Projection-safe identity only: no phone, email, full name, UID, or wallet balance.
  ...
  WITH contributions AS MATERIALIZED (
    SELECT t.id AS transaction_id, t.created_at, -t.coin_amount::bigint AS coins,
      ...
      CASE WHEN t.type::text = 'food' THEN 'food'
           WHEN t.type::text = 'drinks' THEN 'bar' ELSE 'tier_1' END AS source
...
COMMENT ON FUNCTION public.get_dare_board_progress(timestamptz, uuid) IS
  'Admin-only net Tier 1/food/bar coin progress, deterministic net-history milestone pickers and paged contributions. First names/studios only; no contact or band details. Not INR revenue.';
