-- Replay copy of 20260904070518 (Shivek, 4 Sep 2026), moved after 20260904200000 so a FRESH database
-- applies it once the functions it wraps exist. Production already ran the original under version
-- 20260904070518; this copy is idempotent (renames guarded, CREATE OR REPLACE, DROP POLICY IF EXISTS,
-- REVOKE/GRANT) so re-applying it there is a no-op. Guarded on 6 Sep 2026.
...
DO $guard$ BEGIN
  IF to_regprocedure('public._credit_prepaid_coins_to_wallet_unchecked(UUID, UUID)') IS NULL THEN
    ALTER FUNCTION public.credit_prepaid_coins_to_wallet(UUID, UUID) RENAME TO _credit_prepaid_coins_to_wallet_unchecked;
  END IF;
END $guard$;
DO $guard$ BEGIN
  IF to_regprocedure('public._link_wallet_to_event_order_unchecked(UUID, UUID, BOOLEAN)') IS NULL THEN
    ALTER FUNCTION public.link_wallet_to_event_order(UUID, UUID, BOOLEAN) RENAME TO _link_wallet_to_event_order_unchecked;
  END IF;
END $guard$;
DO $guard$ BEGIN
  IF to_regprocedure('public._staff_list_bands_for_order_unchecked(UUID)') IS NULL THEN
    ALTER FUNCTION public.staff_list_bands_for_order(UUID) RENAME TO _staff_list_bands_for_order_unchecked;
  END IF;
END $guard$;
DO $guard$ BEGIN
  IF to_regprocedure('public._reissue_wallet_unchecked(UUID, TEXT, TEXT)') IS NULL THEN
    ALTER FUNCTION public.reissue_wallet(UUID, TEXT, TEXT) RENAME TO _reissue_wallet_unchecked;
  END IF;
END $guard$;
DO $guard$ BEGIN
  IF to_regprocedure('public._phone_has_band_on_order_unchecked(UUID, TEXT)') IS NULL THEN
    ALTER FUNCTION public.phone_has_band_on_order(UUID, TEXT) RENAME TO _phone_has_band_on_order_unchecked;
  END IF;
END $guard$;
DO $guard$ BEGIN
  IF to_regprocedure('public._staff_lookup_party_order_unchecked(TEXT)') IS NULL THEN
    ALTER FUNCTION public.staff_lookup_party_order(TEXT) RENAME TO _staff_lookup_party_order_unchecked;
  END IF;
END $guard$;
