...
Counts negative `coin_amount` transactions for food, drinks (Bar), and games whose transaction category is `tier_1`. Legacy game sales with a null or generic `games` category fall back to the game catalog's current activity group. Explicit historical categories take priority. Voided sales with a linked refund are excluded. Top-ups, coin purchases, other tiers, free games and donation-game spends are excluded. Only first names and studios are returned for guest identification; no phone numbers, emails, full names, wallet balances or NFC IDs. This is a cumulative event ledger total, not a daily total.
