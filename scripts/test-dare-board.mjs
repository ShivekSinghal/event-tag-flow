...
test('reject malformed or inconsistent financial responses instead of showing false progress', () => {
  ...
  for (const changes of [{total_coins:101}, {food_coins:-1}, {bar_coins:'25'}, {as_of:'bad'}, {total_coins:Infinity}]) {
...
test('real Postgres aggregate: category snapshots, no topups, voids, >1000 rows and role checks', async () => {
  ...
  try {
    ...
    assert.deepEqual(Object.keys(before).sort(), ['as_of','bar_coins','counted_sales','food_coins','latest_transactions','log_has_more','milestone_pickers','recent_transactions','tier_1_coins','total_coins']);
